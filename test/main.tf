...
#-------------------- IAM User Groups (Test) ----------------

resource "aws_iam_group" "dimo-dt-user-group-readonly-test" {
  name = "dimo-dt-user-group-readonly-test"
}

resource "aws_iam_group" "dimo-dt-user-group-app-developer-test" {
  name = "dimo-dt-user-group-app-developer-test"
}

resource "aws_iam_group" "dimo-dt-user-group-cloud-developer-test" {
  name = "dimo-dt-user-group-cloud-developer-test"
}

resource "aws_iam_group_policy_attachment" "dimo-dt-user-group-readonly-test-iam-user-change-password" {
  for_each = toset([
    aws_iam_group.dimo-dt-user-group-readonly-test.name,
    aws_iam_group.dimo-dt-user-group-app-developer-test.name,
    aws_iam_group.dimo-dt-user-group-cloud-developer-test.name
  ])
...
resource "aws_iam_group_policy_attachment" "dimo-dt-user-group-readonly-test-read-only-access" {
  for_each = toset([
    aws_iam_group.dimo-dt-user-group-readonly-test.name,
    aws_iam_group.dimo-dt-user-group-cloud-developer-test.name
  ])
...
resource "aws_iam_group_policy" "dimo-dt-user-group-app-developer-test-inline-policy" {
  name  = "SolarCoreServicesAccess"
  group = aws_iam_group.dimo-dt-user-group-app-developer-test.name

  policy = file("${path.module}/policies/solar-core-services-access.json")
}

resource "aws_iam_group_policy" "dimo-dt-user-group-cloud-developer-test-solar-cloud-configuration-access" {
  name  = "SolarCloudConfigurationAccess"
  group = aws_iam_group.dimo-dt-user-group-cloud-developer-test.name
...
resource "aws_vpc_endpoint" "dimo-solar-secretsmanager-vpce-test-apse1" {
  vpc_id              = aws_vpc.dimo-solar-vpc-test-apse1.id
...
# New S3 bucket for test environment.
# Add/adjust policy, lifecycle, CORS, logging, notifications to match source bucket if needed.
resource "aws_s3_bucket" "dimo-solar-s3-bucket-test-apse1" {
  bucket = "dimo-solar-s3-bucket-test-apse1"

  tags = {
    Name      = "dimo-solar-s3-bucket-test-apse1"
    env       = var.env
    terraform = var.terraform
    owner     = var.owner
    project   = var.project
...
resource "aws_s3_bucket_versioning" "dimo-solar-s3-bucket-test-apse1-versioning" {
  bucket = aws_s3_bucket.dimo-solar-s3-bucket-test-apse1.id
...
resource "aws_s3_bucket_ownership_controls" "dimo-solar-s3-bucket-test-apse1-ownership-controls" {
  bucket = aws_s3_bucket.dimo-solar-s3-bucket-test-apse1.id
...
resource "aws_s3_bucket_server_side_encryption_configuration" "dimo-solar-s3-bucket-test-apse1-sse" {
  bucket = aws_s3_bucket.dimo-solar-s3-bucket-test-apse1.id
...
resource "aws_s3_bucket_public_access_block" "dimo-solar-s3-bucket-test-apse1-pab" {
  bucket = aws_s3_bucket.dimo-solar-s3-bucket-test-apse1.id
...
resource "aws_s3_object" "dimo-solar-s3-bucket-test-apse1-folder-upload" {
