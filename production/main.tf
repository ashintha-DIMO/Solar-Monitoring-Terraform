...
#-------------------- IAM User Groups (prod) ----------------

resource "aws_iam_group" "dimo-dt-user-group-readonly-prod" {
  name = "dimo-dt-user-group-readonly-prod"
}

resource "aws_iam_group" "dimo-dt-user-group-app-developer-prod" {
  name = "dimo-dt-user-group-app-developer-prod"
}

resource "aws_iam_group" "dimo-dt-user-group-cloud-developer-prod" {
  name = "dimo-dt-user-group-cloud-developer-prod"
}

resource "aws_iam_group_policy_attachment" "dimo-dt-user-group-readonly-prod-iam-user-change-password" {
  for_each = toset([
    aws_iam_group.dimo-dt-user-group-readonly-prod.name,
    aws_iam_group.dimo-dt-user-group-app-developer-prod.name,
    aws_iam_group.dimo-dt-user-group-cloud-developer-prod.name
  ])
...
resource "aws_iam_group_policy_attachment" "dimo-dt-user-group-readonly-prod-read-only-access" {
  for_each = toset([
    aws_iam_group.dimo-dt-user-group-readonly-prod.name,
    aws_iam_group.dimo-dt-user-group-cloud-developer-prod.name
  ])
...
resource "aws_iam_group_policy" "dimo-dt-user-group-app-developer-prod-inline-policy" {
  name  = "SolarCoreServicesAccess"
  group = aws_iam_group.dimo-dt-user-group-app-developer-prod.name

  policy = file("${path.module}/policies/solar-core-services-access.json")
}

resource "aws_iam_group_policy" "dimo-dt-user-group-cloud-developer-prod-solar-cloud-configuration-access" {
  name  = "SolarCloudConfigurationAccess"
  group = aws_iam_group.dimo-dt-user-group-cloud-developer-prod.name
...
#-------------------- End IAM User Groups (prod) ----------------

# VPC
resource "aws_vpc" "dimo-solar-vpc-prod-apse1" {
...
resource "aws_subnet" "dimo-solar-private1-subnet-apse1a" {
    vpc_id            = aws_vpc.dimo-solar-vpc-prod-apse1.id
...
resource "aws_db_subnet_group" "dimo-solar-rds-subnet-group-prod-apse1" {
...
# Internet Gateway
resource "aws_internet_gateway" "dimo-solar-igw-prod-apse1" {
    vpc_id = aws_vpc.dimo-solar-vpc-prod-apse1.id
...
resource "aws_vpc_endpoint" "dimo-solar-secretsmanager-vpce-prod-apse1" {
  vpc_id              = aws_vpc.dimo-solar-vpc-prod-apse1.id
  service_name        = "com.amazonaws.${var.aws_region}.secretsmanager"
  vpc_endpoint_type   = "Interface"
  subnet_ids          = [aws_subnet.dimo-solar-private1-subnet-apse1a.id, aws_subnet.dimo-solar-private2-subnet-apse1b.id]
  security_group_ids  = [aws_security_group.dimo-solar-vpce-sg-prod-apse1.id]
...
resource "aws_eip" "dimo-solar-nat-eip-prod-apse1" {
  domain = "vpc"
...
resource "aws_nat_gateway" "dimo-solar-nat-gw-prod-apse1" {
  allocation_id = aws_eip.dimo-solar-nat-eip-prod-apse1.id
  ...
  depends_on = [aws_internet_gateway.dimo-solar-igw-prod-apse1]
}

# Route Table for Public Subnets
resource "aws_route_table" "dimo-solar-rtb-public-prod-apse1" {
    vpc_id = aws_vpc.dimo-solar-vpc-prod-apse1.id

    route {
        cidr_block = "0.0.0.0/0"
        gateway_id = aws_internet_gateway.dimo-solar-igw-prod-apse1.id
    }
...
resource "aws_route_table_association" "dimo-solar-public-rtb-assoc" {
  ...
  route_table_id = aws_route_table.dimo-solar-rtb-public-prod-apse1.id
}
...
resource "aws_route_table" "dimo-solar-rtb-private1-apse1a" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.dimo-solar-nat-gw-prod-apse1.id
  }
...
#Create AWS Instance
resource "aws_instance" "dimo-solar-ec2-prod-apse1" {
  ami           = var.ec2_ami
  ...
  vpc_security_group_ids = [aws_security_group.dimo-solar-ec2-sg-prod-apse1.id]
...
resource "aws_lb_target_group" "dimo-solar-alb-tg-prod-apse1" {
...
#Listener for ALB
resource "aws_lb_listener" "dimo-solar-alb-listener-prod-apse1" {
    load_balancer_arn = aws_lb.dimo-solar-alb-prod-apse1.arn
    port              = "80"
    protocol          = "HTTP"

    default_action {
        type             = "forward"
        target_group_arn = aws_lb_target_group.dimo-solar-alb-tg-prod-apse1.arn
    }
}

#Target Group Attachment for ALB
resource "aws_lb_target_group_attachment" "dimo-solar-alb-tg-attachment-prod-apse1" {
    target_group_arn = aws_lb_target_group.dimo-solar-alb-tg-prod-apse1.arn
    target_id        = aws_instance.dimo-solar-ec2-prod-apse1.id
    port             = 3000
}

#Create Application Load Balancer
resource "aws_lb" "dimo-solar-alb-prod-apse1" {
    name               = "${var.project_name}-alb-${var.env}-${var.region_suffix}"
    internal           = false
    load_balancer_type = "application"
    security_groups    = [aws_security_group.dimo-solar-alb-sg-prod-apse1.id]
...
resource "aws_security_group" "dimo-solar-vpce-sg-prod-apse1" {
    vpc_id = aws_vpc.dimo-solar-vpc-prod-apse1.id
...
resource "aws_security_group" "dimo-solar-ec2-sg-prod-apse1" {
    vpc_id = aws_vpc.dimo-solar-vpc-prod-apse1.id
...
resource "aws_security_group" "dimo-solar-rds-sg-prod-apse1" {
    vpc_id = aws_vpc.dimo-solar-vpc-prod-apse1.id
...
resource "aws_security_group" "dimo-solar-alb-sg-prod-apse1" {
    vpc_id = aws_vpc.dimo-solar-vpc-prod-apse1.id
...
resource "aws_security_group" "dimo-solar-lambda-sg-prod-apse1" {
    vpc_id = aws_vpc.dimo-solar-vpc-prod-apse1.id
...
resource "aws_vpc_security_group_egress_rule" "allow-all-to-all-ports" {
    for_each = {
      ec2 = aws_security_group.dimo-solar-ec2-sg-prod-apse1.id
      rds = aws_security_group.dimo-solar-rds-sg-prod-apse1.id
      alb = aws_security_group.dimo-solar-alb-sg-prod-apse1.id
      vpce = aws_security_group.dimo-solar-vpce-sg-prod-apse1.id
    }
...
resource "aws_vpc_security_group_egress_rule" "allow-tcp-to-5432-port" {
    for_each = {
      ec2 = aws_security_group.dimo-solar-ec2-sg-prod-apse1.id
      lambda = aws_security_group.dimo-solar-lambda-sg-prod-apse1.id
    }
    ...
    from_port         = 5432
    to_port           = 5432
    referenced_security_group_id = aws_security_group.dimo-solar-rds-sg-prod-apse1.id
}
...
resource "aws_vpc_security_group_egress_rule" "allow-tcp-to-443-port-from-lambda-and-endpoint" {
    security_group_id = aws_security_group.dimo-solar-lambda-sg-prod-apse1.id
    ip_protocol       = "tcp"
    from_port         = 443
    to_port           = 443 
    referenced_security_group_id = aws_security_group.dimo-solar-vpce-sg-prod-apse1.id
}
...
resource "aws_vpc_security_group_ingress_rule" "allow-all-tcp-from-443" {
    ...
    security_group_id = aws_security_group.dimo-solar-vpce-sg-prod-apse1.id
    ip_protocol       = "tcp"
...
resource "aws_vpc_security_group_ingress_rule" "allow-all-tcp-from-5432" {
    security_group_id = aws_security_group.dimo-solar-ec2-sg-prod-apse1.id
    ip_protocol       = "tcp"
...
resource "aws_vpc_security_group_ingress_rule" "allow-tcp-from-5432" {
    ...
    security_group_id = aws_security_group.dimo-solar-rds-sg-prod-apse1.id
    ip_protocol       = "tcp"
    description = "Allow TCP traffic from port 5432 PostgreSQL"
    from_port         = 5432
    to_port           = 5432 
...
resource "aws_vpc_security_group_ingress_rule" "allow-tcp-to-3000" {
    ...
    from_port         = 3000
    to_port           = 3000 
    referenced_security_group_id = aws_security_group.dimo-solar-alb-sg-prod-apse1.id
}
...
resource "aws_vpc_security_group_ingress_rule" "allow-http-from-anywhere" {
    security_group_id = aws_security_group.dimo-solar-alb-sg-prod-apse1.id
    ip_protocol       = "tcp"
...
# New S3 bucket for prod environment.
# Add/adjust policy, lifecycle, CORS, logging, notifications to match source bucket if needed.
resource "aws_s3_bucket" "dimo-solar-s3-bucket-prod-apse1" {
  bucket = "dimo-solar-s3-bucket-prod-apse1"

  tags = {
    Name      = "dimo-solar-s3-bucket-prod-apse1"
    env       = var.env
    terraform = var.terraform
    owner     = var.owner
    project   = var.project
...
resource "aws_s3_bucket_versioning" "dimo-solar-s3-bucket-prod-apse1-versioning" {
  bucket = aws_s3_bucket.dimo-solar-s3-bucket-prod-apse1.id
...
resource "aws_s3_bucket_ownership_controls" "dimo-solar-s3-bucket-prod-apse1-ownership-controls" {
  bucket = aws_s3_bucket.dimo-solar-s3-bucket-prod-apse1.id
...
resource "aws_s3_bucket_server_side_encryption_configuration" "dimo-solar-s3-bucket-prod-apse1-sse" {
  bucket = aws_s3_bucket.dimo-solar-s3-bucket-prod-apse1.id
...
resource "aws_s3_bucket_public_access_block" "dimo-solar-s3-bucket-prod-apse1-pab" {
  bucket = aws_s3_bucket.dimo-solar-s3-bucket-prod-apse1.id
...
resource "aws_s3_object" "dimo-solar-s3-bucket-prod-apse1-folder-upload" {
